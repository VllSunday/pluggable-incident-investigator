...
- Ошибка action tool или recovery verification сохраняется как evidence и возвращает граф
  к hypotheses → Reflexion → action proposal. Повтор того же idempotency key запрещён, а
  число перепланирований ограничено.
...
Если executor исчерпал retry или действие не восстановило сервис, граф не падает и не
объявляет успех. Он добавляет `action_failure` или `recovery_failure` с provenance, выполняет
один bounded replan и снова применяет policy/HitL к альтернативному действию. При отсутствии
безопасной альтернативы или исчерпании budget инцидент штатно эскалируется.
